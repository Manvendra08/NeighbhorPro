...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const coinBadge = userProfile?.coinBalance != null ? `${userProfile.coinBalance.toLocaleString("en-IN")} NC` : undefined;
  ...
  const SLink = ({ to, icon, label, badge }: { to: string; icon: keyof typeof I; label: string; badge?: string }) => (
    <NavLink to={to} className={lc(to)} title={collapsed ? label : undefined} style={{ position: "relative" }}>
      ...
      {badge && !collapsed && <span style={{ marginLeft: "auto", background: "linear-gradient(135deg,#1B6B8A,#0F4E68)", color: "#fff", fontSize: "0.68rem", fontWeight: 700, padding: "1px 7px", borderRadius: 50 }}>{badge}</span>}
    </NavLink>
  );
        ...
        {!collapsed && (
          <NavLink to="/" style={{ textDecoration: "none", color: "inherit" }}>
            <div className="sidebar-logo" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <img src="/images/logo.png" alt="Logo" style={{ width: "42px", height: "42px", objectFit: "contain", borderRadius: "8px" }} />
              ProNeighbor
            </div>
          </NavLink>
        )}
