...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const lc = (path: string, exact = false) => {
    const active = exact ? location.pathname === path : (location.pathname === path || location.pathname.startsWith(path + "/"));
    return `nav-link${active ? " active" : ""}`;
  };

  const Link = ({ to, icon, label, exact }: { to: string; icon: keyof typeof I; label: string; exact?: boolean }) => (
    <NavLink to={to} className={lc(to, exact)} title={collapsed ? label : undefined}>
      {I[icon]}
  ...
  return (
    <aside className={`sidebar${collapsed ? " collapsed" : ""}`}>
      ...
      <nav className="sidebar-nav">
        ...
        {!isAdmin && <Link to="/dashboard" icon="dashboard" label="Dashboard" exact />}
        <Link to="/browse" icon="browse" label="Browse Pros" />
        <Link to="/messages" icon="messages" label="Messages" />
        <Link to="/account" icon="account" label="My Account" />
        {!isAdmin && <Link to="/support" icon="support" label="Support" />}

        {isAdmin && (
          <>
            {!collapsed && <div className="sidebar-section-label">Overview</div>}
            <Link to="/admin" icon="admin" label="Dashboard" exact />

            {!collapsed && <div className="sidebar-section-label">Users & Content</div>}
            <Link to="/admin/users" icon="users" label="Users" />
            <Link to="/admin/societies" icon="societies" label="Societies" />
            <Link to="/admin/services" icon="services" label="Services" />
            ...
            <Link to="/admin/audit" icon="audit" label="Audit Log" />
            <Link to="/admin/settings" icon="settings" label="Settings" />
          </>
        )}
      </nav>
