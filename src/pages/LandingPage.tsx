  ...
  return (
    <div id="lp-root">
          ...
          <div>
            ...
            <div className="lp-waitlist-counter">
              <div className="lp-progress-bar-wrap">
                <div className="lp-progress-bar" style={{ width: "12%" }} />
              </div>
              <span className="lp-waitlist-label">24 / 200 founding spots claimed</span>
            </div>
      ...
      <section style={{ background: "#fff", padding: `clamp(36px,5vw,60px) ${px}` }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Tag>Launch Use Cases</Tag>
          <h2 className="lp-section-h2">Practical wins Park Street residents can measure in days, not months.</h2>
          <div className="lp-usecases-grid" style={{ gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)" }}>
            {[
              {
                metric: "2.3 hrs saved/week",
                title: "Family admin in one app",
                text: "Parents in Tower C booked a CA + tutor combo and cut weekly coordination from 3 calls to 1 in-app flow.",
              },
              {
                metric: "38% lower monthly spend",
                title: "Group wellness sessions",
                text: "Eight households in Tower A run a shared yoga batch at ₹180/head instead of ₹290 private sessions.",
              },
              {
                metric: "5x faster first booking",
                title: "Hyperlocal trust advantage",
                text: "New residents in Tower D found a verified pediatric consult in under 15 minutes using neighbour endorsements.",
              },
            ].map(({ metric, title, text }) => (
              <div key={title} className="lp-usecase-card">
                <div className="lp-usecase-metric">{metric}</div>
