 ...
 *   users/{uid}.coinBalance          — current balance
 *   coinLedger/{uid}/entries/{id}    — immutable per-user ledger
 *   coinPurchases/{id}               — top-up orders
...
export type LedgerType =
  ...
  | "earn_ondemand"
  | "earn_signup_bonus"
  | "admin_credit"     // admin manually credits NC
  | "admin_debit";     // admin manually debits NC
...
export async function topUpCoins(
  uid: string, priceRs: number, coins: number, packLabel: string, paymentId?: string
): Promise<void> {
  await runTransaction(db, async (tx) => {
    const userRef  = doc(db, "users", uid);
    const userSnap = await tx.get(userRef);
    const current  = (userSnap.data()?.coinBalance as number) ?? 0;
    const newBal   = current + coins;

    tx.update(userRef, { coinBalance: newBal, updatedAt: serverTimestamp() });

    const purchaseRef = doc(collection(db, "coinPurchases"));
    tx.set(purchaseRef, {
      uid, amountPaid: priceRs, coinsGranted: coins, packLabel,
      status: "completed", paymentId: paymentId ?? null,
      createdAt: serverTimestamp(), completedAt: serverTimestamp(),
    } as CoinPurchase);

    const ledgerRef = doc(collection(db, "coinLedger", uid, "entries"));
    tx.set(ledgerRef, {
      uid, type: "topup", amount: coins, balanceAfter: newBal,
      description: `${packLabel} Pack — ₹${priceRs} → ${coins} NC`,
      refId: purchaseRef.id, createdAt: serverTimestamp(),
    } as LedgerEntry);
  });
}
...
export async function payForBooking(
  clientUid: string, proUid: string, bookingId: string,
  coins: number, serviceName: string, platformFeePct = 0.10
  ...
  try {
    await runTransaction(db, async (tx) => {
      ...
      const [clientSnap, proSnap] = await Promise.all([tx.get(clientRef), tx.get(proRef)]);
      const clientBal  = (clientSnap.data()?.coinBalance as number) ?? 0;
      if (clientBal < coins) throw new Error("INSUFFICIENT_BALANCE");
      const proBal     = (proSnap.data()?.coinBalance as number) ?? 0;
      ...
      const cl = doc(collection(db, "coinLedger", clientUid, "entries"));
      tx.set(cl, { uid: clientUid, type: "booking_debit", amount: -coins, balanceAfter: clientBal - coins, description: `Booking: ${serviceName}`, refId: bookingId, createdAt: serverTimestamp() } as LedgerEntry);
      const pl = doc(collection(db, "coinLedger", proUid, "entries"));
      tx.set(pl, { uid: proUid, type: "booking_debit", amount: proEarning, balanceAfter: proBal + proEarning, description: `Earned: ${serviceName} (after 10% fee)`, refId: bookingId, createdAt: serverTimestamp() } as LedgerEntry);
...
export async function refundBooking(clientUid: string, bookingId: string, coins: number, serviceName: string): Promise<void> {
  await runTransaction(db, async (tx) => {
    ...
    tx.update(userRef, { coinBalance: newBal, updatedAt: serverTimestamp() });
    const lr = doc(collection(db, "coinLedger", clientUid, "entries"));
    tx.set(lr, { uid: clientUid, type: "booking_refund", amount: coins, balanceAfter: newBal, description: `Refund: ${serviceName}`, refId: bookingId, createdAt: serverTimestamp() } as LedgerEntry);
...
export async function earnCoins(uid: string, type: LedgerType, refId?: string): Promise<void> {
  ...
  await runTransaction(db, async (tx) => {
    ...
    tx.set(lr, { uid, type, amount: rule.coins, balanceAfter: newBal, description: rule.label, refId: refId ?? null, createdAt: serverTimestamp() } as LedgerEntry);
  });
}
...
export async function requestPayout(uid: string, displayName: string, coins: number, upiId: string): Promise<{ success: boolean; reason?: string }> {
  ...
  try {
    await runTransaction(db, async (tx) => {
      ...
      const lr = doc(collection(db, "coinLedger", uid, "entries"));
      tx.set(lr, { uid, type: "payout", amount: -coins, balanceAfter: newBal, description: `Payout ₹${coins} → ${upiId}`, refId: payoutRef.id, createdAt: serverTimestamp() } as LedgerEntry);
    });
    return { success: true };
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : "";
...
/** All coin purchases (admin view) */
export async function getAllCoinPurchases(pageLimit = 100): Promise<CoinPurchase[]> {
  const q = query(collection(db, "coinPurchases"), orderBy("createdAt", "desc"), limit(pageLimit));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() } as CoinPurchase));
}

/** All payout requests (admin view) */
export async function getAllPayouts(pageLimit = 100): Promise<CoinPayout[]> {
  const q = query(collection(db, "coinPayouts"), orderBy("createdAt", "desc"), limit(pageLimit));
  const snap = await getDocs(q);
...
export async function getPendingPayouts(): Promise<CoinPayout[]> {
  const q = query(collection(db, "coinPayouts"), where("status", "==", "pending"), orderBy("createdAt", "asc"));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() } as CoinPayout));
}

/** Mark a payout as processed or failed */
export async function updatePayoutStatus(
  payoutId: string, status: "processed" | "failed", adminUid: string
): Promise<void> {
  await updateDoc(doc(db, "coinPayouts", payoutId), {
    status, processedBy: adminUid, processedAt: serverTimestamp(),
  });
}

/** Admin manually adjusts a user's NC balance (credit or debit) */
export async function adminAdjustCoins(
  uid: string, amount: number, reason: string, adminUid: string
): Promise<{ success: boolean; reason?: string }> {
  ...
  try {
    await runTransaction(db, async (tx) => {
      ...
      const current  = (snap.data()?.coinBalance as number) ?? 0;
...
/** Platform-wide NC economy summary */
export async function getCoinEconomySummary(): Promise<{
  totalPurchasedNC: number;
  totalPurchaseRevenue: number;
  totalPayoutNC: number;
  totalEarnedNC: number;
  ...
  const pendingPayoutNC     = pendingPayouts.reduce((s, p) => s + p.coinsRedeemed, 0);

  return {
    totalPurchasedNC,
    totalPurchaseRevenue,
    totalPayoutNC,
    totalEarnedNC: 0, // would need collectionGroup query across all ledgers — kept 0 for now
    pendingPayoutNC,
    pendingPayoutCount: pendingPayouts.length,
  };
}

/** User's full ledger (admin — same as user but can query any uid) */
...
export function ledgerColor(type: LedgerType): string {
  if (type.startsWith("earn") || type === "topup" || type === "booking_refund" || type === "admin_credit") return "#16a34a";
  return "#dc2626";
}
