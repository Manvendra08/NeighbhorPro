...
- Name: signup-flow.spec.ts >> Signup Flow >> should display registration page correctly
- Location: e2e\signup-flow.spec.ts:18:3
...
Error: strict mode violation: locator('input[type="password"]') resolved to 2 elements:
    1) <input value="" required="" type="password" placeholder="Min 8 characters"/> aka getByRole('textbox', { name: 'Min 8 characters' })
    2) <input value="" required="" type="password" placeholder="••••••••"/> aka getByRole('textbox', { name: '••••••••' })
