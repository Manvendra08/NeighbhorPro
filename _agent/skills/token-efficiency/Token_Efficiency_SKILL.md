...
## When to Use
- User asks to reduce token usage, prompt size, or context cost
- User wants tighter answers without losing meaning
- Conversation is getting long and context is starting to bloat
- API or model usage cost needs to be reduced

## Procedure
1. Identify what is wasting tokens: prompt text, conversation history, tool output, or response length.
2. Compress the source without removing constraints, decisions, or required details.
3. Replace repetition with references, summaries, or structured bullets.
4. Keep outputs as short as the task allows; use dense formats over prose.
5. At checkpoints, summarize resolved work and retain only unresolved decisions.
6. Verify that quality is unchanged; if precision drops, restore the minimum detail needed.
...
## Completion Checks
- No duplicated instructions or restated context
- No verbose preamble or postamble in responses
- Long inputs summarized before reuse
- Essential constraints preserved intact
